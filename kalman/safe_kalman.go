...
// 	for i := 0; i < c; i++ {
// 		copy(means[i][:], mean.RowView(i).(*mat.VecDense).RawVector().Data)

// 		covMat := mat.NewDense(8, 8, nil)

// 		copy(covMat.RawMatrix().Data, covs[i][:])

// 		left.Mul(df.MotionMat, covMat)
// 		covMat.Mul(left, df.MotionMat.T())
// 		covMat.Add(covMat, motionCov[i])

// 		copy(covs[i][:], covMat.RawMatrix().Data)
// 	}
// }
...
// 	stdsMat := mat.NewDense(len(means), 4, nil)
// 	for i, row := range stds {
// 		stdsMat.SetRow(i, row[:])
// 	}
...
// 	projectedMeansMat := mat.NewDense(c, 4, nil)
...
// 	for i := 0; i < c; i++ {
// 		copy(projectedMeans[i][:], projectedMeansMat.RowView(i).(*mat.VecDense).RawVector().Data)

// 		projectedCovMat := mat.NewDense(4, 4, nil)
...
// 		copy(projectedCovs[i][:], projectedCovMat.RawMatrix().Data)
// 	}
...
// 	projectedMeansMat := mat.NewDense(c, 4, nil)
// 	for i, row := range projectedMeans {
// 		projectedMeansMat.SetRow(i, row[:])
// 	}

// 	measurementsMat := mat.NewDense(c, 4, nil)
// 	for i, row := range measurements {
// 		measurementsMat.SetRow(i, row[:])
// 	}
...
// 	for i := 0; i < c; i++ {
// 		projectedCovMat := mat.NewDense(4, 4, nil)
// 		copy(projectedCovMat.RawMatrix().Data, projectedCovs[i][:])
...
// 	meansMat := mat.NewDense(c, 8, nil)
// 	for i, row := range means {
// 		meansMat.SetRow(i, row[:])
// 	}

// 	meansMat.Add(meansMat, innovations)

// 	for i := range means {
// 		copy(means[i][:], meansMat.RowView(i).(*mat.VecDense).RawVector().Data)
// 	}
// }
